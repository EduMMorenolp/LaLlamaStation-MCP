...
export class MemoryService {
	...
	private ollamaUrl = process.env.OLLAMA_API_URL || "http://127.0.0.1:11434";
	...
	private async embed(model: string, input: string): Promise<number[][]> {
		try {
			...
			console.error("[MemoryService] Error generating embeddings:", error);
			return [];
		}
	}

	private async generate(model: string, prompt: string, options: any = {}): Promise<string> {
		try {
			...
			console.error("[MemoryService] Error generating text:", error);
			throw error;
		}
	}
	...
	async saveMemory(
		...
		try {
			...
			const embeddings = await this.embed(this.embeddingModel, `${title}\n${content}`);
	...
	async updateMemory(id: string, title?: string, content?: string, tags?: string): Promise<boolean> {
		...
		if (title || content) {
			...
			try {
				const embeddings = await this.embed(this.embeddingModel, `${newTitle}\n${newContent}`);
				if (embeddings && embeddings.length > 0) {
					vectorJson = JSON.stringify(embeddings[0]);
				}
			} catch (_err) {}
	...
	async searchMemories(
		...
		if (mode === "semantic" || mode === "hybrid") {
			...
			try {
				const emb = await this.embed(this.embeddingModel, query);
	...
	async suggestTags(model: string, title: string, content: string): Promise<string[]> {
		try {
			...
			const response = await this.generate(model, prompt, { temperature: 0.1, num_ctx: 1024 });
			return response
				.split(",")
...
Does the target memory contradict the existing ones? Respond with YES or NO, followed by a brief explanation.`;
			const response = await this.generate(model, prompt, { temperature: 0.1, num_ctx: 2048 });
...
Analysis:`;
			const analysis = await this.generate(model, prompt, { temperature: 0.1, num_ctx: 2048 });

...
function GaugeRing({ value, max, color, label, unit }: { value: number; max: number; color: string; label: string; unit: string }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '1px' }}>{label}</span>
        </div>
    );
}
    ...
    const saveConfig = async () => {
        ...
        try {
            await Promise.all([
                ...
                axios.post(`${API}/api/engine-stats/cloud-price`, { pricePerMToken: cloudPrice }, { headers: HEADERS() }),
            ]);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div className="card-glass p-8 animate-fade">
                <div className="flex-between" style={{ marginBottom: '24px' }}>
                    ...
                    <button onClick={fetchEngineStats} style={{ background: 'transparent', border: '1px solid var(--border-light)', borderRadius: '8px', padding: '6px 14px', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                {!gpu.vram?.available ? (
                    <div style={{ padding: '20px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '12px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <div>
                            <p style={{ fontWeight: 700, color: '#f59e0b', fontSize: '13px' }}>nvidia-smi no detectado</p>
                            <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>
                                Modo CPU-only. Las metricas de GPU, temperatura y consumo no estan disponibles.
                                Los contadores de tokens si funcionan.
                            </p>
                        </div>
                    </div>
                ) : (
            ...
            <div className="card-glass p-8 animate-fade">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
                    {[
                        { label: 'Tokens Entrada', value: formatTokens(stats.totalInputTokens || 0), icon: '⬇', color: 'var(--text-main)' },
                        { label: 'Tokens Salida', value: formatTokens(stats.totalOutputTokens || 0), icon: '⬆', color: 'var(--accent)' },
                        { label: 'Total Procesado', value: formatTokens(totalTokens), icon: '∑', color: '#10b981' },
                        { label: 'Sesiones', value: (stats.sessionCount || 0).toString(), icon: '💬', color: '#818cf8' },
                    ].map(kpi => (
                        <div key={kpi.label} style={{ padding: '14px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-light)', borderRadius: '10px', textAlign: 'center' }}>
                            <p style={{ fontSize: '22px', marginBottom: '4px' }}>{kpi.icon}</p>
                            <p style={{ fontSize: '18px', fontWeight: 800, fontFamily: 'var(--font-mono)', color: kpi.color }}>{kpi.value}</p>
                            <p style={{ fontSize: '9px', color: 'var(--text-muted)', marginTop: '4px', textTransform: 'uppercase', letterSpacing: '1px' }}>{kpi.label}</p>
                        </div>
                    ))}
                </div>
                ...
                <div style={{ background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '12px', overflow: 'hidden' }}>
                    <div style={{ padding: '14px 20px', background: 'rgba(16,185,129,0.08)', borderBottom: '1px solid rgba(16,185,129,0.15)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <Award size={16} style={{ color: '#10b981' }} />
                        <span style={{ fontSize: '12px', fontWeight: 800, color: '#10b981', letterSpacing: '1px' }}>BALANCE — LOCAL vs CLOUD</span>
                    </div>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ borderBottom: '1px solid var(--border-light)' }}>
                                {['Metrica', 'LaLlamaStation 🦙', 'Cloud (OpenAI/DeepSeek)'].map(h => (
                                    <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '10px', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {[
                                ...
                                <tr key={row.metric} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                                    <td style={{ padding: '10px 16px', fontSize: '12px', color: 'var(--text-dim)' }}>{row.metric}</td>
                                    <td style={{ padding: '10px 16px', fontSize: '12px', fontWeight: 700, color: 'var(--text-main)', fontFamily: 'var(--font-mono)' }}>{row.local}</td>
                                    <td style={{ padding: '10px 16px', fontSize: '12px', color: 'var(--text-muted)' }}>{row.cloud}</td>
                                </tr>
                            ))}
                            {/* Fila de balance final */}
                            <tr style={{ background: 'rgba(16,185,129,0.08)' }}>
                                <td style={{ padding: '12px 16px', fontWeight: 800, fontSize: '13px', color: '#10b981' }}>BALANCE FINAL</td>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', alignItems: 'center' }}>
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '11px', color: 'var(--text-muted)' }}>
                            ...
                            <span style={{ color: thermalColor, fontWeight: 700 }}>{stats.thermalStressScore || 0} pts</span>
                        </div>
                        <div style={{ width: '100%', height: '10px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', overflow: 'hidden', marginBottom: '12px' }}>
                            <div style={{ width: `${thermalPct}%`, height: '100%', background: thermalColor, boxShadow: `0 0 10px ${thermalColor}`, transition: 'width 0.5s ease, background 0.3s' }} />
                        </div>
                        <p style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.6' }}>
                            Cada minuto sobre 75°C suma 1 punto. Sobre 85°C suma 3 puntos.<br />
                            Las GPUs modernas toleran bien hasta 83°C en carga sostenida.
                        </p>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {[
                            { label: 'Horas de Inferencia', value: `${(stats.inferenceHours || 0).toFixed(2)}h`, icon: <Clock size={14} /> },
                            { label: 'Sessions Procesadas', value: (stats.sessionCount || 0).toString(), icon: <BarChart2 size={14} /> },
                            { label: 'kWh Consumidos (est.)', value: `${(stats.kwhConsumed || 0).toFixed(4)} kWh`, icon: <Zap size={14} /> },
                        ].map(item => (
                            <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-light)', borderRadius: '8px' }}>
                                ...
                                <span style={{ fontSize: '13px', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{item.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="card-glass p-8 animate-fade">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div>
                        <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '1px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '20px' }}>
                    <button
                        className="auth-btn" disabled={saving}
                    ...
                    {savedMsg && (
                        <span style={{ fontSize: '12px', fontWeight: 700, color: savedMsg.startsWith('✓') ? 'var(--success)' : 'var(--error)' }}>
                            {savedMsg}
                        </span>
                    )}
                </div>

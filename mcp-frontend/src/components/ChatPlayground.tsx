...
function CopyButton({ text }: { text: string }) {
    ...
    return (
        <button onClick={handleCopy} style={{
            ...
            padding: '2px 4px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '3px',
...
export const ChatPlayground: React.FC<ChatPlaygroundProps> = ({ models, onSendMessage }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 160px)', minHeight: '500px', gap: 0 }}>
            <style>{`
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    {loading ? <RefreshCw size={18} style={{ color: 'var(--accent)', animation: 'spin 1s linear infinite' }} /> : <Bot size={18} style={{ color: 'var(--accent)' }} />}
                </div>

                {/* Info del modelo */}
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <h3 style={{ fontSize: '14px', fontWeight: 800, color: 'var(--text-main)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {modelShortName || 'Sin modelo'}
                        </h3>
                        <span style={{
                            fontSize: '9px', padding: '2px 8px', borderRadius: '4px', fontWeight: 800, letterSpacing: '1px',
                ...
                {totalTokensSession > 0 && (
                    <div style={{ display: 'flex', gap: '16px', flexShrink: 0 }}>
                        <div style={{ textAlign: 'center' }}>
                            <p style={{ fontSize: '13px', fontWeight: 800, color: 'var(--accent)', fontFamily: 'var(--font-mono)' }}>
                                {totalTokensSession > 1000 ? `${(totalTokensSession / 1000).toFixed(1)}K` : totalTokensSession}
                            </p>
                            <p style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>tokens</p>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                            <p style={{ fontSize: '13px', fontWeight: 800, color: 'var(--success)', fontFamily: 'var(--font-mono)' }}>
                                {formatLatency(totalTimeSession)}
                            </p>
                            <p style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>sesion</p>
                        </div>
                    </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                    {history.length > 0 && (
                        <button className="btn-icon" onClick={clearChat} title="Limpiar chat" style={{ color: 'var(--text-muted)' }}>
                            <Trash2 size={16} />
                        </button>
                    )}
                    <button
            ...
            {showSettings && (
                <div style={{
                    padding: '16px 20px', background: 'rgba(10,10,20,0.95)',
                    borderBottom: '1px solid var(--border-light)', flexShrink: 0,
                    display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '16px', alignItems: 'end',
                }}>
                    <div>
                        <label style={{ fontSize: '10px', color: 'var(--text-muted)', display: 'block', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '1px' }}>
                    ...
                    <div>
                        ...
                        >
                            {[2048, 4096, 8192, 16384, 32768, 65536].map(v => (
                                <option key={v} value={v}>{v >= 1024 ? `${v / 1024}K` : v} tokens</option>
                            ))}
                        </select>
                    </div>
                </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                {history.length === 0 && !loading && (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '24px' }}>
                        <div style={{ textAlign: 'center', opacity: 0.15 }}>
                            ...
                            <p style={{ fontSize: '13px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase' }}>LaLlamaStation Playground</p>
                            <p style={{ fontSize: '11px', marginTop: '4px', opacity: 0.7 }}>Inferencia local con Ollama</p>
                        </div>
                            ...
                            {suggestions.map(s => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        textAlign: 'left', lineHeight: '1.4', transition: 'var(--transition)',
                                    }}
                        ...
                        <div className={`${msg.role === 'user' ? 'bubble-user' : `bubble-bot${msg.isError ? ' bubble-error' : ''}`}`}>
                            ...
                            <div className="msg-footer-bar">
                                ...
                                {msg.role === 'assistant' && msg.latencyMs && (
                                    <span className="msg-meta" style={{ color: msg.latencyMs > 10000 ? '#f59e0b' : 'var(--text-muted)' }}>
                                        <Zap size={9} />
                ...
                {loading && (
                    <div className="chat-msg-bot">
                        <div className="bubble-bot" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <TypingDots />
                            <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                    ...
                    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                        <select
                            ...
                            style={{
                                ...
                                borderRadius: '6px', color: 'var(--accent)', fontSize: '11px', fontWeight: 700,
                                padding: '3px 24px 3px 8px', cursor: 'pointer', outline: 'none',
                                appearance: 'none',
                            }}
                        >
                            {models.length === 0
                        ...
                        <ChevronDown size={10} style={{ position: 'absolute', right: '6px', color: 'var(--accent)', pointerEvents: 'none' }} />
                    </div>

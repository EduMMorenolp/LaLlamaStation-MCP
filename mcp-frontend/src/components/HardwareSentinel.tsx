...
import { Cpu, Zap, Timer, SlidersHorizontal, AlertTriangle, CheckCircle, XCircle, RefreshCw, Save, Power } from 'lucide-react';
...
function VramBadge({ modelSizeBytes, vram }: { modelSizeBytes: number; vram: any }) {
    ...
    if (modelMb > vramFreeMb * 0.8 && modelMb <= vramFreeMb) {
        ...
        icon = '🟡'; label = 'Ajustado'; desc = 'Cerca del límite de VRAM';
    } else if (modelMb > vramFreeMb) {
        color = 'var(--error)'; bg = 'rgba(239,68,68,0.1)'; border = 'rgba(239,68,68,0.3)';
        icon = '🔴'; label = 'Híbrido (CPU)'; desc = 'Irá más lento (offload a RAM)';
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div className="card-glass p-8 animate-fade">
                ...
                {!vram?.available ? (
                    <div style={{ padding: '24px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                            <p style={{ fontSize: '13px', fontWeight: 700, color: '#f59e0b' }}>nvidia-smi no detectado</p>
                            <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px', lineHeight: '1.5' }}>
                                Ollama está en modo CPU-only o en macOS (Metal). Los modelos se ejecutan en RAM del sistema.<br />
                    ...
                    <>
                        ...
                        <div style={{ width: '100%', height: '8px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', overflow: 'hidden', marginBottom: '8px' }}>
                            <div style={{
                                ...
                                width: `${vramUsedPct}%`,
                                background: vramUsedPct > 90 ? 'var(--error)' : vramUsedPct > 70 ? '#f59e0b' : 'var(--accent)',
                ...
                {loadedModels.length > 0 && (
                    <div style={{ marginTop: '16px' }}>
                        <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '1px' }}>Cargado en VRAM ahora</p>
                        {loadedModels.map((m: any) => (
                            <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', background: 'rgba(79,140,255,0.06)', border: '1px solid rgba(79,140,255,0.2)', borderRadius: '8px', marginBottom: '8px' }}>
                                ...
                                {vram?.available && m.size && (
                                    <VramBadge modelSizeBytes={m.size} vram={vram} />
                                )}
                            </div>
                        ))}
                    </div>
            ...
            <div className="card-glass p-8 animate-fade">
                ...
                <div style={{ padding: '12px 16px', background: 'rgba(79,140,255,0.05)', border: '1px dashed rgba(79,140,255,0.3)', borderRadius: '8px', fontSize: '11px', color: 'var(--text-dim)' }}>
                    💡 Para descargar con cuantización, usa el nombre completo en la sección Modelos: <code style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent)' }}>llama3.2:{selectedQuant}</code>
                </div>
            </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                            {AUTO_UNLOAD_OPTIONS.map(opt => (
                                <button
                                    ...
                                    style={{
                                        padding: '8px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontWeight: 700,
                                        background: autoUnload === opt.value ? 'rgba(79,140,255,0.15)' : 'rgba(255,255,255,0.03)',
                    ...
                    <div>
                        ...
                        <div style={{ marginBottom: '12px' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: 'var(--text-muted)', marginTop: '4px' }}>
                                <span>512</span>
                                <span style={{ fontSize: '14px', fontWeight: 800, color: 'var(--accent)', fontFamily: 'var(--font-mono)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                            {[2048, 4096, 8192, 16384, 32768].map(v => (
                                <button key={v} onClick={() => setNumCtx(v)} style={{
                                    padding: '4px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '10px', fontWeight: 700,
